...
__global__ void compute_keys_mode_ma(uint32_t mode, uint8_t *bloomLookUp, int BLOOM_BITS, uint8_t BLOOM_HASHES,
									 uint64_t *keys, uint32_t maxFound, uint32_t *found, curandState *states)
{
	...
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	uint64_t random_key = curand(&states[idx]);

	ComputeKeysSEARCH_MODE_MA(mode, keys + xPtr, keys + yPtr, bloomLookUp, BLOOM_BITS, BLOOM_HASHES, maxFound, found, random_key);
}
...
__global__ void compute_keys_comp_mode_ma(uint32_t mode, uint8_t *bloomLookUp, int BLOOM_BITS, uint8_t BLOOM_HASHES, uint64_t *keys,
										  uint32_t maxFound, uint32_t *found, curandState *states)
{
...
// mode single address
__global__ void compute_keys_mode_sa(uint32_t mode, uint32_t *hash160, uint64_t *keys, uint32_t maxFound, uint32_t *found, curandState *states)
{
	...
	ComputeKeysSEARCH_MODE_SA(mode, keys + xPtr, keys + yPtr, hash160, maxFound, found, random_key);
}

__global__ void compute_keys_comp_mode_sa(uint32_t mode, uint32_t *hash160, uint64_t *keys, uint32_t maxFound, uint32_t *found, curandState *states)
{
										  ...
										  uint32_t maxFound, uint32_t *found, curandState *states)
{
	...
	ComputeKeysSEARCH_MODE_MX(mode, keys + xPtr, keys + yPtr, bloomLookUp, BLOOM_BITS, BLOOM_HASHES, maxFound, found, random_key);
}

// mode single x point
__global__ void compute_keys_comp_mode_sx(uint32_t mode, uint32_t *xpoint, uint64_t *keys, uint32_t maxFound, uint32_t *found, curandState *states)
{
	...
	ComputeKeysSEARCH_MODE_SX(mode, keys + xPtr, keys + yPtr, xpoint, maxFound, found, random_key);
}
										 ...
										 uint32_t maxFound, uint32_t *found, curandState *states)
{
	...
	ComputeKeysSEARCH_ETH_MODE_MA(keys + xPtr, keys + yPtr, bloomLookUp, BLOOM_BITS, BLOOM_HASHES, maxFound, found, random_key);
}

__global__ void compute_keys_mode_eth_sa(uint32_t *hash, uint64_t *keys, uint32_t maxFound, uint32_t *found, curandState *states)
{
	...
	ComputeKeysSEARCH_ETH_MODE_SA(keys + xPtr, keys + yPtr, hash, maxFound, found, random_key);
}
					 ...
					 uint8_t BLOOM_HASHES, const uint8_t *BLOOM_DATA, uint8_t *DATA, uint64_t TOTAL_COUNT, bool rKey)
{
	...
	CudaSafeCall(cudaMalloc((void **)&randomStates, nbThread * sizeof(curandState)));

	// Initialize random states
	init_random_state<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(randomStates, time(NULL));
...
GPUEngine::~GPUEngine()
{
	...
	// Free random states
	CudaSafeCall(cudaFree(randomStates));
...
bool GPUEngine::callKernelSEARCH_MODE_MA()
{
	...
	if (coinType == COIN_BTC)
	{
		...
		{
			compute_keys_comp_mode_ma<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer, randomStates);
		}
		else
		{
			compute_keys_mode_ma<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer, randomStates);
		}
	}
	else
	{
		compute_keys_mode_eth_ma<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer, randomStates);
	}
...
bool GPUEngine::callKernelSEARCH_MODE_MX()
{
	...
	if (compMode == SEARCH_COMPRESSED)
	{
		compute_keys_comp_mode_mx<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer, randomStates);
	}
	else
	{
...
bool GPUEngine::callKernelSEARCH_MODE_SA()
{
	...
	if (coinType == COIN_BTC)
	{
		if (compMode == SEARCH_COMPRESSED)
		{
			compute_keys_comp_mode_sa<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputHashORxpoint, inputKey, maxFound, outputBuffer, randomStates);
		}
		else
		{
			compute_keys_mode_sa<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputHashORxpoint, inputKey, maxFound, outputBuffer, randomStates);
		}
	}
	else
	{
		compute_keys_mode_eth_sa<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(inputHashORxpoint, inputKey, maxFound, outputBuffer, randomStates);
	}
...
bool GPUEngine::callKernelSEARCH_MODE_SX()
{
	...
	if (compMode == SEARCH_COMPRESSED)
	{
		compute_keys_comp_mode_sx<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputHashORxpoint, inputKey, maxFound, outputBuffer, randomStates);
	}
	else
	{
		printf("GPUEngine: PubKeys search doesn't support uncompressed\n");
...
bool GPUEngine::LaunchSEARCH_MODE_SX(std::vector<ITEM> &dataFound, bool spinWait)
{
	...
	return callKernelSEARCH_MODE_S