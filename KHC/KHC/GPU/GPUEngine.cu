...
__global__ void SetRandomKeysKernel(uint64_t *randomKeys)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	curandState state;
	curand_init(randomKeys[idx], idx, 0, &state);
}
...
void GPUEngine::InitializeRandomKeys(uint64_t *randomKeys)
{
	// Allocate memory for random keys
	uint64_t *deviceRandomKeys;
	CudaSafeCall(cudaMalloc((void **)&deviceRandomKeys, nbThread * sizeof(uint64_t)));

	// Copy random keys to device memory
	CudaSafeCall(cudaMemcpy(deviceRandomKeys, randomKeys, nbThread * sizeof(uint64_t), cudaMemcpyHostToDevice));

	// Free the allocated memory on the host
	delete[] randomKeys;

	// Set the random keys in the kernels
	SetRandomKeysKernel<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(deviceRandomKeys);

	// Free device memory
	CudaSafeCall(cudaFree(deviceRandomKeys));
}
...
std::string GPUEngine::GetDeviceName() const
{
	return deviceName;
}
...
bool GPUEngine::callKernelSEARCH_MODE_MA()
{
	...
	if (coinType == COIN_BTC)
	{
		...
		{
			compute_keys_comp_mode_ma<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer);
		}
		else
		{
			compute_keys_mode_ma<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer);
		}
	}
	else
	{
		compute_keys_mode_eth_ma<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer);
	}
...
bool GPUEngine::callKernelSEARCH_MODE_MX()
{
	...
	if (compMode == SEARCH_COMPRESSED)
	{
		compute_keys_comp_mode_mx<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputBloomLookUp, BLOOM_BITS, BLOOM_HASHES, inputKey, maxFound, outputBuffer);
	}
	else
	{
...
bool GPUEngine::callKernelSEARCH_MODE_SA()
{
	...
	if (coinType == COIN_BTC)
	{
		...
		{
			compute_keys_mode_sa<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputHashORxpoint, inputKey, maxFound, outputBuffer);
		}
	}
	else
	{
		compute_keys_mode_eth_sa<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(inputHashORxpoint, inputKey, maxFound, outputBuffer);
	}
...
bool GPUEngine::callKernelSEARCH_MODE_SX()
{
	...
	if (compMode == SEARCH_COMPRESSED)
	{
		compute_keys_comp_mode_sx<<<nbThread / nbThreadPerGroup, nbThreadPerGroup>>>(compMode, inputHashORxpoint, inputKey, maxFound, outputBuffer);
	}
	else
	{
		printf("GPUEngine: PubKeys search doesn't support uncompressed\n");
