...
void KeyHunt::FindKeyGPU(TH_PARAM *ph)
{
    ...
    uint64_t *randomKeys = new uint64_t[nbThread];
    for (int i = 0; i < nbThread; i++)
    {
        randomKeys[i] = rand(); // Initialize with random values
    }
    g->InitializeRandomKeys(randomKeys);
    ...
    while (ok && !endOfSearch)
    {
        ...
        if (ph->rKeyRequest)
        {
            ...
            // Initialize random keys again
            for (int i = 0; i < nbThread; i++)
            {
                randomKeys[i] = rand(); // Initialize with new random values
            }
            g->InitializeRandomKeys(randomKeys);
        }
